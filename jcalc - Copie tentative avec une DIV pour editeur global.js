...
// Table //////////////////////////////////////////////////////////////
  ...
  Table.prototype.span = function(options) {
    ...
    var h = '<table'/*+(this._code?' id="'+this._name+'" class="EDITOR"':'')*/+'><thead><tr>';  // TODO modify to style
    for (var col in options.cols) {
      h += '<th>'+col+'</th>';
    }
    h += '</tr></thead><tbody>';    
      ...
      for (var col in options.cols) {
        ...
        if (options.cols[col] != 0) {
          ...
          var attrs = (c.style?'style="'+c.style+'"':'')+
                      ...
                      ' row="'+i+'" col='+JSON.stringify(col);     //**** TODO verifier que STRINGIFY est ok pour cela en particulier si le string contient "
...
a(event.type,event.target.tagName,event.target.row,event.target.col);
    return false; // prevent bubbling
  }

  Table.prototype.edit = function(options) {
    this._code = jc.output._codeElement;   // premier jet: toute la table est éditée peut être simple et efficasse: en cas de tables partielles, faire simplement plusieurs tables et faire une fct pour lier plusieurs tables
    $(this._code).addClass('AUTOEDIT');
   
    return jc.html('<DIV class=EDITOR id='+JSON.stringify(this._name)+' contentEditable=true">'+this.span(options)+'</DIV>');
