...
const callback = function (mutationsList, observer) {
  for (let mutation of mutationsList) {
    if (mutation.type === 'childList') {
      ...
      console.info('node ' + $(mutation.target).attr('id') + ' >  The ' + mutation.attributeName + ' attribute was modified. new:' + $(mutation.target).attr(mutation.attributeName) + ' old:' + mutation.oldValue);
    } else if (mutation.type === 'characterData') {
      console.info('node ' + mutation.target.nodeType + ' >  character changed:' + $(mutation.target).text() + ' old:' + mutation.oldValue + ' selection:'+window.getSelection().toString());
    }
    else {
      console.info('mutation.type=' + mutation.type);
    }
  }
};
...
function startObserver() {
  const config = {attributes: true, childList: true, subtree: true, attributeOldValue: true,characterData:true,characterDataOldValue:true};
// Start observing the target node for configured mutations
  observer.observe(tb.tbContent$[0], config);

  console.info('started the observer');
}

function stopObserver() {
  observer.disconnect();
  console.info('stopped the observer');
}